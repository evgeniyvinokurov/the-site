...
class ArrayWorker {
	...
	ReGetResult(itemField, descriptions) {
		...
		if (descriptions.fAll) {
			result = descriptions.fAll(itemField)		
		}
	...
	Sum(code){
		this.fAll = new Function("items", code);
		return this.fAll(this.data);
	}
}
Random = {};
...
jQuery(document).ready(function(){
	...
	var htmlDescriptions = function(analyzed, parent = false) {
		...
		if (!parent)
			html += '<input type="submit" name="sum" value="sum"/>';
	...
	$(document).on('click', "input[type='submit'][name='donesum']", function() {
		var selfParent = $(this).parent();
		var text = selfParent.find('textarea[name="codeforsum"]').val();		

		alert("Data can be applied: " + JSON.stringify(aw.Sum(text)));		
	})
	...
	$(document).on('click', "input[type='submit'][name='sum']", function() {
		var parent = $(this).parent();
		parent.append("<textarea name='codeforsum'>var result = {}; for(var item of items){  }; return result;</textarea><input type='submit' value='done' name='donesum' />")				
	})
});