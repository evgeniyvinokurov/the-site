...
Tetris = function(speed, matrixSize, figureSize, cellSize){
	...
	this.Figure = function(dimentions, coordinates, colors, empty = false, matrix){
		...
		this.move = function(direction){
			...
			var newRightGipoteticalBoundary = x + selfFigure.dimentions[0] + direction[0];
			var newLeftGipoteticalBoundary = x + direction[0];

			if ((newLeftGipoteticalBoundary < selfFigure.matrix.dimentions[0] - 1) && (newLeftGipoteticalBoundary > -1)) {
				x += direction[0];
			}
	...
	this.Matrix = function(dimentions, coordinates, colors, figureOptions) {
		...
		this.fallFigures = function(){
			for (var fi of selfMatrix.fallingFigures) {
				if(selfMatrix.moveLeft){
					fi.moveLeft();
					selfMatrix.moveLeft = false;
				}

				if(selfMatrix.moveRight){
					fi.moveRight();
					selfMatrix.moveRight = false;
				}

				if(selfMatrix.moveDown){
					selfMatrix.moveDown = false;
	...
	this.moveLeft = function(){
		selfTetris.ba.matrix.moveLeft = true;
	}
	this.moveRight = function(){
		selfTetris.ba.matrix.moveRight = true;		
	}
	this.goDown = function(){
		selfTetris.ba.matrix.moveDown = true;		
	}
