...
let PicsService = {
    ...
    mainContext.arc(ToolsService.getRandom(0, width), ToolsService.getRandom(0, height), ToolsService.getRandom(10, 12),  0, 2 * Math.PI, true);
    mainContext.lineWidth = ToolsService.getRandom(5, 10);
    mainContext.strokeStyle = ToolsService.getRandomColor();
    mainContext.stroke();
	},
	drawRect:function(mainContext){
	 var size = ToolsService.getRandom(0, 10)
	 mainContext.beginPath();
	...
	drawLine: function(ctx){
     let coords = ToolsService.getGoodLine()
    console.log(coords)
	 ctx.beginPath();
	 ctx.moveTo(coords.x1, coords.y1); 
	 ctx.lineTo(coords.x2, coords.y2);
    ctx.lineWidth = ToolsService.getRandom(5, 10);
    ctx.strokeStyle = ToolsService.getRandomColor();
	 ctx.stroke();
  	},
